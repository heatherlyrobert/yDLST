...
yDLST_greybox      (char *a_question, int  a_index)
{
   ...
   if        (strncmp(a_question, "list_end_ptr" , 20)   == 0) {
      ...
      xlist = h_lists; while (xlist != NULL) { ++xfore; xlist = xlist->f_lists; }
      xlist = t_lists; while (xlist != NULL) { ++xback; xlist = xlist->b_lists; }
   ...
   /*---(list link dll)------------------*/
      ...
      xlink = xlist->head; while (xlink != NULL) { ++xfore; xlink = xlink->fore; }
      xlink = xlist->tail; while (xlink != NULL) { ++xback; xlink = xlink->back; }
